...
func TestPost_GetPost(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(expectedSQL)).WithArgs(1).
		WillReturnRows(sqlmock.NewRows([]string{"pid", "writer", "title", "content", "time_to_read", "create_at"}).AddRow(1, "writer1", "title1", "content1", 1, currentTime))
	...
	assert.Equal(t, int32(1), post.TimeToRead)
...
func TestPost_GetBulkPost(t *testing.T) {
	...
	assert.Equal(t, int32(1), post[0].TimeToRead)
...
func TestPost_InsertPost(t *testing.T) {
	...
	expectedSQL := fmt.Sprintf("INSERT IGNORE INTO board_post(writer, title, content, time_to_read) VALUES (?, ?, ?, ?)")
	mock.ExpectExec(regexp.QuoteMeta(expectedSQL)).WithArgs(post.Writer, post.Title, post.Content, post.TimeToRead).WillReturnResult(sqlmock.NewResult(1, 1))
